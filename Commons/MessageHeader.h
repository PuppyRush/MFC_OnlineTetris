...
namespace msg_header
{
	...
	typedef struct mSendName : public Header
	{
		...
		mSendName(const Header h, const int namelen, const char *name)
	...
	typedef struct mSendNames : public Header
	{
		...
		mSendNames(const Header h, const size_t enternum, const size_t *namelen, const char (*names)[ID_LEN] )
	...
	typedef struct mSendMessage :public Header
	{
		...
		mSendMessage(const Header h, const size_t msglen, const char* msg)
			:Header(h)
		{
	...
	typedef struct mSendReady : public Header
	{
		...
		mSendReady(const Header h, const size_t namelen, const char* fromname, const bool ready)
			:Header(h)
		{
			this->ready = ready;
	...
	typedef struct mSendRadies : public Header
	{
		...
		mSendRadies(const Header h, const bool *ready, const int enternum, const size_t *namelen, const char (*name)[ID_LEN])
			:Header(h), enternum(enternum)
		{
	...
	typedef struct mSendStartsignal : public Header
	{
		...
		mSendStartsignal(const Header h, const int map, const int level, const bool ghost, const bool gravity)
	...
	typedef struct mSendAddline : public Header
	{
		...
		mSendAddline(const Header h, size_t namelen, char *name, size_t linenum)
